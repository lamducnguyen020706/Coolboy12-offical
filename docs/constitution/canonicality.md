...
This artifact answers one question: **what "canonical" means in each of the six sovereign Record
Models, and what canonicality is not.**
...
This document is AUTHORITATIVE about **the meaning of canonicality in each Record Model, and its
boundaries**, and about nothing else. It derives both from the Master Blueprint and the Record Model
System; it does not amend, supersede, or outrank either. Where this document differs from the Master
...
**In scope.** What canonical means in W, E, P, R, V and I; that P and I are never canonical; that V
is canonical by kind; what canonicality is not; that canonicality does not cross model boundaries on
its own.
...
observation"* (§13.7c). Visual canonicality is decided per Kind, not for the model as a whole:

| Visual Kind (RMS §11.1, `FROZEN`) | Canonicality | Source |
|---|---|---|
...
The governing reason is Blueprint §18.6: *"canonical visual identity is the description, not the
file."* An asset rendered from a canonical specification does not become canonical by being rendered
from it; a divergence between them *"is a continuity finding against the asset"* (§13.6c). RMS §11.1
admits these three Kinds only; visual reference and visual derivative are not Kinds.
...
**Why no universal boolean can carry this table.** Two of the six have no canonical status at all,
so a `false` in them would assert a status the model does not have; and one of the six is decided
per Kind, so no single value could state it. §13.7c draws the same consequence for status
vocabularies: *"A `status` vocabulary admitting `CANON` in every partition would be false in two of
six"*.
...
§10.1). Canonicality is the status, defined per model, of what counts as canonical. Neither implies
the other in general: *authorized ⇒ canonical* and *authority ⇒ canonicality* are not rules. The
authority framework is Artifact 051, and this contract does not restate it.
...
| 20 | Storage of canonical status | RMS §4 |

## 10. Dependency and Ownership

| Concern | Owner |
|---|---|
| What canonical means, per model | **This contract** — the six meanings of its §6, from §13.7c |
| Elaboration of one model's canonicality | That model's own contract |
| How a Record becomes canonical — gate, ceremony, transitions | Artifact 145 and each model's own contract |
...
models`). A model's own contract may elaborate its canonicality; it may not redefine the meaning
fixed here, turn a *Never* into a *Yes*, or apply its meaning to another model.

**Source condition recorded.** Blueprint §18.6 still says the canonical visual specification *"is a
World record, gated like any other canon"*. Blueprint §13.6c (v0.6.1) and RMS §11 (`FROZEN`) place
it in the Visual partition, and §13.6c records the move: a canonical visual identity *"was a World
record (Section 18.6)"*, a split *"correct about authority and wrong about location"*. This contract
follows §13.6c and RMS §11. §18.6's location wording is the Blueprint's to correct, and is not
corrected here; its principle — the description, not the file — is unaffected.
...
| **C-052-06** | Visual canonicality is by kind; only the specification is canonical. | Blueprint §13.7c, §13.6c; RMS §11.1 |
...
| **C-052-10** | No universal canonical boolean, enum, status vocabulary or field is established. | Blueprint §13.7c; Roadmap row 052 |
...
| Visual: three Kinds; specification canonical; asset a manifestation; analysis an observation | RMS §11.1, §11.2; Blueprint §13.6c |
...
document states what canonical means in each Record Model, derived from Master Blueprint §13.7c and
RMS §17 with the model sections that corroborate them. It does not amend them. It is not a Record,
holds no canonical data, and defines no gate, lifecycle, field or schema. Where it differs from the
Master Blueprint, the Record Model System, or the OS File Build Roadmap, those governing sources are
